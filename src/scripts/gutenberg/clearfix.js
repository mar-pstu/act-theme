( function( blocks, editor, i18n, element, components, _ ) {
    var el = element.createElement;

    blocks.registerBlockType( 'pstu-theme/clearfix', {
        title: i18n.__( 'PSTU Clearfix', 'act_theme' ),
        description: i18n.__( 'Используется для решения проблем, возникающих при содержании «плавающих» элементов в рамках блока контейнера или обтекания элементов', 'act_theme' ),
        keywords: [
            i18n.__( 'ПГТУ', 'act_theme' ),
            i18n.__( 'очистка', 'act_theme' ),
            i18n.__( 'ластик', 'act_theme' ),
            i18n.__( 'обтекание', 'act_theme' ),
            i18n.__( 'изображение', 'act_theme' ),
            i18n.__( 'выравнивание', 'act_theme' ),
        ],
        icon: el(
            'img', {
                'src': "data:image/svg+xml;base64,",
            }
        ),
        category: 'layout',

        edit: function( props ) {
            return el(
                'div', {
                    className: 'pstu-clearfix ' + props.className
                },
                el(
                    'div', {
                        className: 'pstu-clearfix-inner'
                    },
                    el(
                        'span', {
                            className: 'label'
                        },
                        'clearfix'
                    )
                ),
            );
        },

        save: function( props ) {
            return el( 'div', {
                className: 'clearfix'
            } );
        },
    } );

} )(
    window.wp.blocks,
    window.wp.editor,
    window.wp.i18n,
    window.wp.element,
    window.wp.components,
    window._,
);